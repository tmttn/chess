...
<div
  ...
  class:legal-target={isLegalTarget}
...
<style>
  ...
  .light.last-move {
    background: color-mix(in srgb, var(--square-light), var(--square-last-move));
  }

  .dark.last-move {
    background: color-mix(in srgb, var(--square-dark), var(--square-last-move));
  }
  ...
  .legal-dot {
    ...
    height: 30%;
    border-radius: 50%;
    background: var(--square-legal);
  }

  .legal-ring {
    position: absolute;
    inset: 0;
    border: 4px solid var(--square-legal);
    border-radius: 50%;
    pointer-events: none;
  }
</style>
